...
static void md5_block(MD5_CTX *c, uint32_t *X, int num)
{
	uint32_t A,B,C,D;
...
void MD5_Update(MD5_CTX *c, const register uint8_t *data, uint32_t len)
{
	uint32_t *p;
	int sw,sc;
	uint32_t l;
...
#define l2c(l,c)    (*((c)++)=(uint8_t)(((l)     )&0xff), *((c)++)=(uint8_t)(((l)>> 8L)&0xff), *((c)++)=(uint8_t)(((l)>>16L)&0xff), *((c)++)=(uint8_t)(((l)>>24L)&0xff))

void MD5_Final(uint8_t *md, MD5_CTX *c)
{
	int i,j;
	uint32_t l;
	uint32_t *p;
	uint8_t end[4]={0x80,0x00,0x00,0x00};
	uint8_t *cp=end;
...
void Md5HashBuffer(const void* inBuffer, uint32_t inBufferLen,  uint8_t *outBuffer)
{
	...
	MD5_Update( &md5InfoBuffer, (uint8_t*)inBuffer, inBufferLen );
...
void Bin2Hex(const void* inBuffer, uint32_t inBufferLen, uint8_t* outHexBuffer, int lowCase)
{
    for(int i = 0; i < inBufferLen; i++){
        if(lowCase) {
            sprintf(outHexBuffer + i * 2, "%02x", ((uint8_t *) inBuffer)[i]);
        }else{
            sprintf(outHexBuffer + i * 2, "%02X", ((uint8_t *) inBuffer)[i]);
        }
    }
